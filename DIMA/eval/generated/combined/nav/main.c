...
/* ---- Inter-partition sampling ports ---- */
...
void appMain(void)
{
    ...
    /* ---- Create inter-partition sampling ports ---- */

    CREATE_SAMPLING_PORT("pos_out", sizeof(integer), SOURCE,
                         100000000ll, &(nav_pos_out_id), &ret);
    CHECK_CODE("CREATE_SAMPLING_PORT(pos_out)", ret);

    CREATE_SAMPLING_PORT("status_out", sizeof(integer), SOURCE,
                         100000000ll, &(nav_status_out_id), &ret);
    CHECK_CODE("CREATE_SAMPLING_PORT(status_out)", ret);

    /* ---- Create inter-partition queuing port ---- */

    CREATE_QUEUING_PORT("cmd_out", sizeof(integer), 4, SOURCE, FIFO,
                        &(nav_cmd_out_id), &ret);
    CHECK_CODE("CREATE_QUEUING_PORT(cmd_out)", ret);

    /* ---- Create intra-partition blackboards ---- */

    CREATE_BLACKBOARD("bb_bb1", sizeof(integer), &(nav_bb_bb1_id), &ret);
    CHECK_CODE("CREATE_BLACKBOARD(bb_bb1)", ret);

    CREATE_BLACKBOARD("bb_bb2", sizeof(integer), &(nav_bb_bb2_id), &ret);
    CHECK_CODE("CREATE_BLACKBOARD(bb_bb2)", ret);

    /* ---- Create intra-partition buffer ---- */

    CREATE_BUFFER("buf_buf1", sizeof(integer), 4, FIFO,
                  &(nav_buf_buf1_id), &ret);
